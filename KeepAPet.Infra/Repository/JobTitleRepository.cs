...
namespace KeepAPets.Infra.Repository
{
    public class JobTitleRepository:IJobTitleRepository
    {
        ...
        public JobTitleRepository(IDBContext dbContext)
        {
            DBContext = dbContext;
        }
        public int Create(JobTitle Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);
           
            p.Add("@JobTitleType", Data.JobTitleType, dbType: DbType.String, direction: ParameterDirection.Input);
          
            var result = DBContext.Connection.ExecuteAsync("AddJobTitle", p, commandType: CommandType.StoredProcedure);
            return 1;

        }
        public List<JobTitle> GetAll()
        {
            IEnumerable<JobTitle> result = DBContext.Connection.Query<JobTitle>("GetAllJobTitle", commandType: CommandType.StoredProcedure);
            return result.ToList();
            //var result = DBContext.Connection.ExecuteAsync("GetAllEmployee", commandType: CommandType.StoredProcedure);
            //return 1;

        }
        public int Update(JobTitle Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);

            p.Add("@JobTitleType", Data.JobTitleType, dbType: DbType.String, direction: ParameterDirection.Input);
            var result = DBContext.Connection.ExecuteAsync("EditJobTitle", p, commandType: CommandType.StoredProcedure);
            return 1;
        }
