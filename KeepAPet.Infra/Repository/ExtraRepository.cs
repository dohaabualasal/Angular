...
namespace KeepAPets.Infra.Repository
{
    public class ExtraRepository:IExtraRepository
    {
        ...
        public ExtraRepository(IDBContext dbContext)
        {
            DBContext = dbContext;
        }
        public int Create(Extra Data)
        {
            ...
            p.Add("@Type", Data.Type, dbType: DbType.String, direction: ParameterDirection.Input);
            p.Add("@ClinkId", Data.ClinkId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            var result = DBContext.Connection.ExecuteAsync("AddExtra", p, commandType: CommandType.StoredProcedure);
            return 1;

        }
        public List<Extra> GetAll()
        {
            IEnumerable<Extra> result = DBContext.Connection.Query<Extra>("GetAllExtra", commandType: CommandType.StoredProcedure);
            return result.ToList();
            //var result = DBContext.Connection.ExecuteAsync("GetAllEmployee", commandType: CommandType.StoredProcedure);
            //return 1;

        }
        public int Update(Extra Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);
            p.Add("@price", Data.price, dbType: DbType.Double, direction: ParameterDirection.Input);
            p.Add("@Type", Data.Type, dbType: DbType.String, direction: ParameterDirection.Input);
            p.Add("@ClinkId", Data.ClinkId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            var result = DBContext.Connection.ExecuteAsync("EditExtra", p, commandType: CommandType.StoredProcedure);
            return 1;
        }
