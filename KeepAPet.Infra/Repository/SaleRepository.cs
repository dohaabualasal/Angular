...
namespace KeepAPets.Infra.Repository
{
    public class SaleRepository:ISalesRepository
    {
        ...
        public int Create(Sales Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);

            p.Add("@AccountantId", Data.AccountantId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            p.Add("@PaymentId", Data.PaymentId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            p.Add("@Quantity", Data.Quantity, dbType: DbType.Int32, direction: ParameterDirection.Input);
            ...
            var result = DBContext.Connection.ExecuteAsync("AddSale", p, commandType: CommandType.StoredProcedure);
            return 1;

        }
        public List<Sales> GetAll()
        {
            IEnumerable<Sales> result = DBContext.Connection.Query<Sales>("GetAllSale", commandType: CommandType.StoredProcedure);
            return result.ToList();
            //var result = DBContext.Connection.ExecuteAsync("GetAllEmployee", commandType: CommandType.StoredProcedure);
            //return 1;

        }
        public int Update(Sales Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);
            ...
            p.Add("@price", Data.price, dbType: DbType.Double, direction: ParameterDirection.Input);
            p.Add("@Date", Data.Date, dbType: DbType.DateTime, direction: ParameterDirection.Input);
            var result = DBContext.Connection.ExecuteAsync("EditSale", p, commandType: CommandType.StoredProcedure);
            return 1;
        }
