...
namespace KeepAPets.Infra.Repository
{
    public class FoodRepository:IFoodRepository
    {
        ...
        public int Create(Food Data)
        {
            ...
            var result = DBContext.Connection.ExecuteAsync("AddFood", p, commandType: CommandType.StoredProcedure);
            return 1;

        }
        public List<Food> GetAll()
        {
            IEnumerable<Food> result = DBContext.Connection.Query<Food>("GetAllFood", commandType: CommandType.StoredProcedure);
            return result.ToList();
            //var result = DBContext.Connection.ExecuteAsync("GetAllEmployee", commandType: CommandType.StoredProcedure);
            //return 1;

        }
        public int Update(Food Data)
        {
            var p = new DynamicParameters();
            p.Add("@Id", Data.Id, dbType: DbType.Int32, direction: ParameterDirection.Input);
            p.Add("@price", Data.price, dbType: DbType.Double, direction: ParameterDirection.Input);
            p.Add("@Type", Data.Type, dbType: DbType.String, direction: ParameterDirection.Input);
            p.Add("@ClinkId", Data.ClinkId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            var result = DBContext.Connection.ExecuteAsync("EditFood", p, commandType: CommandType.StoredProcedure);
            return 1;
        }
